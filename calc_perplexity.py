...
def calculate_perplexity(model, tokenizer, input_text, device, batch_size):
...
device = "cuda:1"
model_id = "/network/damian/hackathon/chatmusician_pruned50_oneshot"
model = SparseAutoModelForCausalLM.from_pretrained(model_id, device_map=device)
...
dataset_list = [sample["prompt"] + sample["text"] for sample in raw_dataset]
results = calculate_perplexity(
    model=model,
    tokenizer=tokenizer,
    input_text = dataset_list,
    device=device,
    batch_size=4
)
